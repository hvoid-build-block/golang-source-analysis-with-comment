...
func makeBucketArray(t *maptype, b uint8, dirtyalloc unsafe.Pointer) (buckets unsafe.Pointer, nextOverflow *bmap) {
	...
	if b >= 4 {
		...
		up := roundupsize(sz) // fix size sz to page size
		if up != sz {
			nbuckets = up / t.bucket.size // re-calculate buckets number
		}
	}

	if dirtyalloc == nil {
		buckets = newarray(t.bucket, int(nbuckets)) // allocates an array of n elements of type typ, typ has size value
	} else {
...
func mapassign(t *maptype, h *hmap, key unsafe.Pointer) unsafe.Pointer {
	...
	// check if build with -race, check if there is race condition in map assignment.
	if raceenabled {
		callerpc := getcallerpc()
		pc := funcPC(mapassign)
